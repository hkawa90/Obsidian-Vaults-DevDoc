## 文書作成

### 画面キャプチャー系
- [GitHub - stokedowl/terminalizer-v2: 🦄 Record your terminal and generate animated gif images or share a web player](https://github.com/stokedowl/terminalizer-v2)
- [ScreenToGif - 画面を録画し、編集して GIF、ビデオ、またはその他の形式で保存します。](https://www.screentogif.com/)
- [Chrome Capture -Screenshot＆GIF - Chrome ウェブストア](https://chromewebstore.google.com/detail/chrome-capture-screenshot/ggaabchcecdbomdcnbahdfddfikjmphe)
- [ShareX - The best free and open source screenshot tool for Windows](https://getsharex.com/)

### Draw/Paint
- [Draw Freely | Inkscape](https://inkscape.org/ja/)
  イラストレーター、デザイナー、ウェブデザイナーなど、ベクターイメージを必要としている皆様に、Inkscape をオススメします。
  ![](https://www.youtube.com/watch?v=rFYQW2DCM2I&t=46s)
  ![](https://www.youtube.com/watch?v=AAhVqLyKk4U)
- [GIMP - GNU Image Manipulation Program](https://www.gimp.org/)
  ![](https://www.youtube.com/watch?v=_L_MMU22bAw&t=8s)
- [draw.io](https://www.drawio.com/)
  ブラウザ/デスクトップ上で動作するオープンソースの図表作成ツール
  ![](https://www.youtube.com/watch?v=XS11brUtj70&t=3s)
### File Manager
- [ranger/ranger: A VIM-inspired filemanager for the console](https://github.com/ranger/ranger)
  ![[Pasted image 20241024231644.png]]
- [jarun/nnn: n³ The unorthodox terminal file manager](https://github.com/jarun/nnn?tab=readme-ov-file)
- [Stow - GNU Project - Free Software Foundation](https://www.gnu.org/software/stow/stow.html)
  GNU Stowは、ファイルシステム上の別々のディレクトリにあるソフトウェアやデータの別々のパッケージを、同じ場所にインストールされているように見せるシンボリックリンク・ファーム・マネージャーである。
- [lsd-rs/lsd: The next gen ls command](https://github.com/lsd-rs/lsd)
  ![[Pasted image 20241024232846.png]]

### Editor
- [Visual Studio Code - Windows に無料でダウンロードしてインストールする | Microsoft Store](https://apps.microsoft.com/detail/xp9khm4bk9fz7q?launch=true&mode=full&hl=ja-jp&gl=jp&ocid=bingwebsearch)
- [Obsidian - Sharpen your thinking](https://obsidian.md/)
- [GNU Emacs - GNU Project](https://www.gnu.org/software/emacs/)

### Resource Monitor
- top: procps package
- [htop - an interactive process viewer](https://htop.dev/)
- [aristocratos/btop: A monitor of resources](https://github.com/aristocratos/btop)
  ![[Pasted image 20241024231534.png]]
- [dylanaraps/neofetch: 🖼️ A command-line system information tool written in bash 3.2+](https://github.com/dylanaraps/neofetch)
  現在public archived.
  ![[Pasted image 20241024231841.png]]
- [Using the Linux ncdu command to view your disk usage | Network World](https://www.networkworld.com/article/972132/using-the-linux-ncdu-command-to-view-your-disk-usage.html)

### Web
- [jarun/googler: :mag: Google from the terminal](https://github.com/jarun/googler)
  現在public archived.
  ![[Pasted image 20241024232355.png]]
  

### Terminal
- [Warp: The intelligent terminal](https://www.warp.dev/)
  AI搭載Rust製ターミナル
- [GitHub - GitSquared/edex-ui: A cross-platform, customizable science fiction terminal emulator with advanced monitoring & touchscreen support.](https://github.com/GitSquared/edex-ui)
  Coolなんだけど、現在public archived.
  ![[Pasted image 20241026174146.png]]
### Misc
- [junegunn/fzf: :cherry\_blossom: A command-line fuzzy finder](https://github.com/junegunn/fzf)
  ![[Pasted image 20241024232624.png]]

